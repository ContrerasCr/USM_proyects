...
def save_bbdd(razon_social, rut, rubro, direccion, contacto, mail, fono, celda):
    ...
    try:
        core['E' + str(celda)] = contacto
    except ValueError:
        print('Error', 'E', celda)
        pass

    try:
        core['F' + str(celda)] = mail
    except ValueError:
        print('Error', 'F', celda)
        pass

    try:
        core['G' + str(celda)] = fono
    except ValueError:
        print('Error', 'G', celda)
        pass
...
def completar_faltante(mercantil, mercadopublico, webportal, genealog, svs, celda):

    datos = ['None', 'None', 'None', 'None', 'None', 'None', 'None', celda]

    mercantil = minusculas(mercantil)
    mercadopublico = minusculas(mercadopublico)
    webportal = minusculas(webportal)
    genealog = minusculas(genealog)
    svs = minusculas(svs)

    print(mercantil[0], mercadopublico[0], webportal[0], genealog[0], svs[0], 'razon social')

    dat = [mercantil, mercadopublico, webportal, genealog, svs]
    reales = [mercantil, mercadopublico, webportal, genealog, svs]
...
try:
    ...
    for coor in range(score, len_core):
        ...
        if val[0] == 10000000:
            ...
            print(val[0], val[1], val[2], 'fill', 'A' + str(coor), 'iniciales')
            ...
            if re.search(r'No se han encontrado resultados para tu', src):
                ...
                driver.close()
                completar_faltante(datos_mercantil, datos_mercado_publico, datos_webportal, datos_genealog, datos_svs, coor)
    print('End')
